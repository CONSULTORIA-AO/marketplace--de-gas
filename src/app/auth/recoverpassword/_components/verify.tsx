    ...
    >
      <div className="text-center space-y-2">
        <motion.div
          ...
          className="w-16 h-16 sm:w-20 sm:h-20 mx-auto rounded-full gradient-[#FFA500] flex items-center justify-center mb-4"
        >
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="flex justify-center gap-2 sm:gap-3">
          {code.map((digit, index) => (
            <motion.input
              ...
              className="w-10 h-12 sm:w-12 sm:h-14 md:w-14 md:h-16 text-center text-lg sm:text-xl md:text-2xl font-semibold rounded-lg border-2 border-[#FFA500] bg-card bg-white text-black focus:border-[#FFA500] focus:ring-2 focus:ring-[#FFA500]/20 outline-none transition-all"
            />
          ))}
        </div>
        ...
        <Button
          ...
          className="w-full h-12 rounded-lg bg-green-600 hover:bg-green-700 text-white disabled:opacity-50"
        >
      ...
      <div className="text-center">
        {canResend ? (
          <button
            ...
            className="text-white hover:underline text-sm sm:text-base inline-flex items-center gap-2"
          >
            <RefreshCw className="w-4 h-4" />
            Reenviar código
          </button>
        ) : (
          <p className="text-[#FFA500] text-sm sm:text-base">
            Reenviar em{' '}
            <span className="font-semibold text-[#FFA500]">{countdown}s</span>
          </p>
        )}
      </div>
      ...
      <Button
        ...
        variant="outline"
        onClick={onBack}
        className="w-full h-12 sm:h-14 rounded-lg text-sm text-white sm:text-base border-0 bg-[#FFA500] hover:bg-green-700 text-white"
      >
