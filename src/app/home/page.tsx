    ...
    <div className="min-h-screen bg-[#F5F5F5] font-sans antialiased">
      ...
      >
        <div className="max-w-7xl mx-auto px-4 flex flex-wrap justify-center md:justify-between items-center gap-4 text-sm">
          {[
            ...
            <motion.div key={item.text} whileHover={{ y: -2 }} className="flex items-center gap-2 text-gray-600">
              <span>{item.icon}</span>
      ...
      <section className="max-w-7xl mx-auto px-4 pb-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { bg: "from-[#1259C3] to-[#0A3D8F]", title: "Frete grátis", sub: "Em produtos selecionados", icon: "🚚", cta: "Ver agora" },
            { bg: "from-[#FFA500] to-[#FFD000]", title: "Oferta do dia", sub: "Até 60% de desconto", icon: "⚡", cta: "Aproveitar", dark: true },
            { bg: "from-[#00A650] to-[#007A3A]", title: "Parcele em 12x", sub: "Sem juros no cartão", icon: "💳", cta: "Saiba mais" },
          ].map((b, i) => (
            <motion.div
              key={i}
            ...
            >
              <div>
                <p className={`font-black text-xl ${b.dark ? "text-gray-900" : "text-white"}`}>{b.title}</p>
                <p className={`text-sm mt-1 ${b.dark ? "text-gray-700" : "text-white/80"}`}>{b.sub}</p>
              </div>
      ...
      {/* ── STATS ────────────────────────────────────────── */}
      <section ref={statsRef} className="bg-gradient-to-br from-[#FFA500] to-[#FFA500] py-16 my-8">
      ...
      <section className="px-6" id="newsletter">
        <h2 className="text-3xl font-semibold mb-6 text-[#FFA500] text-center">Receba Novidades</h2>
