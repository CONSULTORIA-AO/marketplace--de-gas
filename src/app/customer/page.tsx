...
export default function Customer() {
  ...
  const [user, setUser] = useState<User>({
    name:"João Manuel Silva", email:"joao.silva@gmail.com", phone:"+244 923 456 789",
    address:"Rua da Missão, 45, Luanda", city:"Luanda", country:"Angola",
    avatar:"https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=200&q=80",
    memberSince:"Janeiro 2023", plan:"Gratuito",
  });
  ...
  const updateQty = (id: number, delta: number): void => setCart(prev =>
  ...
  const toggleFav = (product: Product): void => {
    setFavorites(prev => prev.find(i => i.id === product.id) ? prev.filter(i => i.id !== product.id) : [...prev, product]);
  };
  ...
  const filtered: Product[] = ALL_PRODUCTS
  ...
  return (
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif", background:"#F0F4FF", minHeight:"100vh"}}>

      {notification && (
        <div style={{position:"fixed",top:16,right:16,zIndex:9999,background:BLUE,color:"white",
          padding:"12px 20px",borderRadius:12,fontSize:13,fontWeight:600,
          boxShadow:"0 8px 24px rgba(18,89,195,0.3)",animation:"slideIn .3s ease"}}>
          {notification}
        </div>
      )}
      ...
      {sidebarOpen && (
        <div style={{position:"fixed",inset:0,zIndex:400,display:"flex"}}>
          ...
          <Sidebar user={user} cart={cartCount} favorites={favorites.length}
            goTo={goTo} close={() => setSidebar(false)} currentView={view}/>
        </div>
      )}
      ...
      <div style={{maxWidth:1400,margin:"0 auto",padding:"16px 16px 80px"}}>
        {view === "shop" && (
          <ShopView
            products={filtered} categories={CATEGORIES} activeCategory={activeCategory}
            setCategory={setCategory} sortBy={sortBy} setSort={setSort}
            priceRange={priceRange} setPriceRange={setPriceRange}
            filterOpen={filterOpen} setFilterOpen={setFilterOpen}
            addToCart={addToCart} toggleFav={toggleFav} favorites={favorites}
            onProductClick={p => { setSelectedProduct(p); setView("productDetail"); }}
            onPayNow={p => { setPaymentItem(p); setView("payment"); }}
          />
        )}
        {view === "productDetail" && selectedProduct && (
          <ProductDetail
            ...
            addToCart={addToCart} toggleFav={toggleFav} favorites={favorites}
            onChat={p => { setChatSeller(p); setView("chat"); }}
            onPayNow={p => { setPaymentItem(p); setView("payment"); }}
            onBack={() => setView("shop")}
          />
        )}
        {view === "cart" && (
          <CartView
            ...
            cartTotal={cartTotal}
            onCheckout={() => { setPaymentItem("cart"); setView("payment"); }}
            onBack={() => setView("shop")}
          />
        )}
        {view === "payment" && paymentItem && (
          <PaymentView
            item={paymentItem === "cart" ? { name:`${cartCount} itens no carrinho`, price:cartTotal } : paymentItem}
            cart={paymentItem === "cart" ? cart : null}
            onSuccess={() => { if (paymentItem === "cart") setCart([]); notify("Pagamento efectuado com sucesso! ✓"); setView("orders"); }}
            onBack={() => setView(paymentItem === "cart" ? "cart" : "productDetail")}
          />
        )}
        {view === "favorites" && (
          <FavoritesView
            favorites={favorites} addToCart={addToCart} toggleFav={toggleFav}
            onProductClick={p => { setSelectedProduct(p); setView("productDetail"); }}
            onBack={() => setView("shop")}
          />
        )}
        {view === "orders"  && <OrdersView orders={ORDERS} onBack={() => setView("shop")}/>}
        {view === "chat"    && <MessagesView onBack={() => setView("shop")}/>}
        {view === "profile" && <ProfileView user={user} setUser={setUser} onBack={() => setView("shop")}/>}
        {view === "settings"&& <SettingsView user={user} setUser={setUser} onBack={() => setView("shop")} notify={notify}/>}
        {view === "subs"    && <SubscriptionsView subs={SUBS} user={user} setUser={setUser} notify={notify} onBack={() => setView("shop")}/>}
      </div>
