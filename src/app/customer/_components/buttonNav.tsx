...
export function BottomNav({ view, goTo, cartCount }: BottomNavProps) {
  ...
  return (
    <div style={{position:"fixed",bottom:0,left:0,right:0,background:"white",borderTop:"1px solid #E5E7EB",display:"flex",zIndex:200,boxShadow:"0 -4px 16px rgba(0,0,0,0.08)"}}>
      {items.map(({ icon, label, v, badge }) => (
        <button key={v} onClick={() => goTo(v)}
          style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",padding:"8px 0",background:"none",border:"none",cursor:"pointer",position:"relative"}}>
          <Icon name={icon} color={view === v ? BLUE : "#9CA3AF"} size={20}/>
          <span style={{fontSize:10,marginTop:2,color:view===v?BLUE:"#9CA3AF",fontWeight:view===v?700:400}}>{label}</span>
