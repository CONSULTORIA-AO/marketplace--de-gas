        ...
        {subs.map((s) => (
          <div
            ...
            style={{
              ...
              borderRadius: 16,
              padding: 24,
              border: `2px solid ${activeSub === s.id ? s.color : '#E5E7EB'}`,
              boxShadow:
                activeSub === s.id
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                padding: '10px',
                borderRadius: 10,
                background: activeSub === s.id ? s.color : WHITE,
                border: `1.5px solid ${activeSub === s.id ? s.color : ORANJE}`,
                color: activeSub === s.id ? 'white' : ORANJE,
                fontWeight: 700,
                fontSize: 14,
                cursor: 'pointer',
              }}
      ...
      >
        ...
        >
          <div>
            ...
            <p style={{ fontSize: 13, color: '#6B7280', margin: 0 }}>
              Subscreva para receber alertas do que quiser
            </p>
          </div>
          <button
        ...
        {showCustom && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <Field
              label="O que quer receber alertas?"
              placeholder="ex: iPhone, Apartamentos T3, Nike..."
              icon="bell"
              value={customSub.prefs}
              onChange={(v) => setCustomSub((c) => ({ ...c, prefs: v }))}
            />
            <Field
              label="Nome"
              ...
              onChange={(v) => setCustomSub((c) => ({ ...c, name: v }))}
            />
            <Field
              label="E-mail para alertas"
              placeholder="email@exemplo.com"
              icon="bell"
              value={customSub.email}
              onChange={(v) => setCustomSub((c) => ({ ...c, email: v }))}
            />
            <button
              onClick={() => {
